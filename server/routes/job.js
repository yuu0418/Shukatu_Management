...
app.post("/register", async (req, res) => {
  ...
  try {
    await pool.query(
      "INSERT INTO jobs (job_id, user_id , job_name, status, nextStep , memo , tags , dueDate ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
...
app.post("/delete", async (req, res) => {
  ...
  try {
    await pool.query("DELETE FROM jobs WHERE job_id = $1 AND user_id = $2", [
      job_id,
      user_id,
    ]);
...
app.post("/update", async (req, res) => {
  ...
  try {
    await pool.query(
      "UPDATE jobs SET job_name = $1, status = $2, nextStep = $3, memo = $4, tags = $5, dueDate = $6 updated_at = CURRENT_TIMESTAMP FROM job_id = $7 AND user_id = $8",
